...
class Soldier(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
        ...
        self.speed = speed
        self.base_speed = speed  # Store the base speed for resetting
        self.ammo = ammo
        self.start_ammo = ammo
        self.shoot_cooldown = 0
        self.grenades = grenades
        self.health = 150  # Increased for better survival
        ...
        # Q-learning variables for player
        self.q_table = {}
        self.state = 0
        self.epsilon = 0.1
        self.alpha = 0.1
        ...
        # Auto-jump variables
        self.jump_cooldown = 0  # Prevent spamming jumps
        self.double_jump_available = False  # Track if double jump is available
        self.last_jump_time = 0  # Track time since last jump for double jump window
        ...
        for tile in world.obstacle_list:
            ...
            if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
                if self.vel_y < 0:
                    ...
                    self.vel_y = 0
                    self.in_air = False
                    self.double_jump_available = False  # Reset double jump on landing
    ...
    def shoot(self):
        if self.shoot_cooldown == 0 and self.ammo > 0:
            self.shoot_cooldown = 20 if self.char_type == 'player' else 30
            bullet = Bullet(self.rect.centerx + (0.75 * self.rect.size[0] * self.direction), self.rect.centery, self.direction)
            bullet_group.add(bullet)
            self.ammo -= 1
            shot_fx.play()

    def auto_jump(self, obstacles):
        if self.char_type != 'player' or self.jump_cooldown > 0:
            return
        ...
        detection_rect = pygame.Rect(self.rect.right, self.rect.y - jump_height_threshold, look_ahead, jump_height_threshold + self.rect.height)
        obstacle_detected = False
        for tile in obstacles:
            if tile[1].colliderect(detection_rect):
                if tile[1].top < self.rect.bottom and tile[1].bottom > self.rect.bottom - jump_height_threshold:
                    obstacle_detected = True
                    break

        # Check for gaps (pits) by looking for absence of ground tiles below
        ground_check_rect = pygame.Rect(self.rect.left - 30, self.rect.bottom, look_ahead + (self.rect.right - self.rect.left) + 30, TILE_SIZE // 4)
        ground_detected = False
        for tile in obstacles:
            if tile[1].colliderect(ground_check_rect):
                ground_detected = True
                break

        # Check for water tiles ahead
        water_check_rect = pygame.Rect(self.rect.right - 20, self.rect.bottom - 10, look_ahead + 20, 30)
        water_detected = False
        for water in water_group:
            if water.rect.colliderect(water_check_rect):
                water_detected = True
                break

        # Double jump check while in air
        ...
        # Trigger initial jump if an obstacle, gap, or water is detected
    ...
    def update_q_table(self, state, action, reward, next_state):
        max_next_q = max(self.q_table[next_state][0], self.q_table[next_state][1])
...
while run:
    ...
    if start_game == False:
        ...
        if player.alive:
            ...
            # Auto-jump over obstacles and pits
            player.auto_jump(world.obstacle_list)
            ...
            # Updated Q-learning reward system
            reward = 0
            current_bullet_count = len(bullet_group)
            if current_bullet_count < last_bullet_count:
                reward = -1  # Penalty for shooting
            last_bullet_count = current_bullet_count

            # Add reward for hitting enemies
            for enemy in enemy_group:
            ...
            if player.last_action is not None:
                ...
                if nearest_enemy:
                    next_min_distance = math.sqrt(((player.rect.centerx + player.speed) - enemy.rect.centerx)**2 + (player.rect.centery - enemy.rect.centery)**2)
                next_state = 0 if next_min_distance < 100 else 1 if next_min_distance < 300 else 2
                player.update_q_table(player.state, player.last_action, reward, next_state)
